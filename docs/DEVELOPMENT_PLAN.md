...
为体现"具身机器大脑"的核心理念，进行了以下调整：

- **Phase 顺序调整**：先进行软调试（聊天交互验证大脑能力），再进行硬件集成
  - Phase 5.1: 聊天交互与软技能调试 (✅ 已完成)
  - Phase 5.2: 真机集成测试与调试 (🔄 待完成，强调身体能力识别)

- **系统架构扩展**：SYSTEM_ARCHITECTURE.md 添加了"🧠 具身机器大脑设计理念"完整章节，包含四大设计原则：
  1. **能力感知** - 大脑启动时查询身体的真实能力
  2. **能力约束** - 所有行为基于身体的实际能力
  3. **优雅降级** - 缺失能力时提供替代方案而非失败
  4. **能力通告** - 向用户清晰说明能力和限制

- **核心理念**：大脑必须**适应身体的能力**，而不是反过来

详见 [SYSTEM_ARCHITECTURE.md](SYSTEM_ARCHITECTURE.md#-具身机器大脑设计理念) 的设计理念章节。
...
**目标**: 在真实Unitree G1/Go2机器人硬件上进行完整的系统集成测试。关键原则是：**大脑必须对身体的能力进行感知和适应**。大脑根据身体的能力来决定如何行动，而不是反过来。例如：如果身体上没有摄像头，大脑就不能主动"看"世界，只能通过用户输入的图像进行分析。

**设计原则**:
1. **能力感知**: 大脑在启动时必须查询身体的能力清单
2. **能力约束**: 所有行为必须基于身体的实际能力
3. **优雅降级**: 如果某些能力不可用，提供替代方案而不是失败
4. **能力通告**: 向用户清晰地说明当前身体的能力和限制

**关键任务**:

#### 5.2.1 身体能力识别与约束系统 🔄 待完成
- [ ] 身体能力检节点
  - [ ] 在连接时自动检测机器人的硬件配置
  - [ ] 查询摄像头、麦克风、传感器等可用资源
  - [ ] 记录运动能力范围（DOF、最大速度、力度等）
  - [ ] 建立身体能力的实时能力清单
- [ ] 大脑-身体能力映射
  - [ ] PrefrontalCortex生成意图时必须检查身体能力
  - [ ] VisualCortex如果身体无摄像头则禁用主动视觉
  - [ ] MotorCortex只生成身体能够执行的代码
  - [ ] 记录哪些功能被禁用及原因
- [ ] 能力约束验证
  - [ ] 在执行前验证动作是否超出身体限制
  - [ ] 防止要求"看"的指令在无摄像头时执行
...
#### 5.2.2 真机SDK命令集成 🔄 待完成
- [ ] 硬件连接与初始化
  - [ ] 建立与Unitree机器人的网络连接 (UDP/TCP)
  - [ ] 实现机器人状态查询和固件版本验证
  - [ ] 获取身体能力配置并缓存到系统
  - [ ] 设置SDK认证和安全通信
- [ ] 运动控制验证
  - [ ] 测试基础运动命令 (前进、后退、旋转、站立、坐下)
  - [ ] 验证速度和加速度控制参数
  - [ ] 实现平滑运动轨迹规划
  - [ ] 测试平衡控制和姿态调整
- [ ] 操纵能力测试
...
- [ ] 备选方案（无摄像头）
  - [ ] 大脑改为要求用户提供图像
  - [ ] "你能给我看看周围的环境吗？" → 等待用户上传图像
  - [ ] 支持API端点接收外部图像源
  - [ ] 清楚地告知用户"我没有摄像头，请告诉我看到什么"
...
✅ **Milestone 8: Chat Interface 与软技能系统完成** (完成 100% - 2026.04.03)
- 聊天交互系统: ChatInterface 类 (720+ 行, 功能完整)
- 6 个核心软技能: math, joke, write, explain, remember, learn
- Chat Interface 测试: 6/6 软技能通过 (100% 通过率)
- 与核心模块集成: PrefrontalCortex, Hippocampus, Cerebellum, IntegrationEngine
- LLM 集成: Graceful fallback 支持
- 用户界面: Rich CLI 美观交互界面
- 启动脚本: `python scripts/chat.py` 可直接使用
- 预期用途: 验证大脑核心智能能力，不依赖物理身体
- 为 Phase 5.2 真机集成积累经验

🔄 **下一步**: Phase 5.2 (真机集成测试 - 大脑与身体的能力适配)
